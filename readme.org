...
method, please write to me at <martianh@disroot.org> and I can
...
#+BEGIN_SRC emacs-lisp :results table :colnames '("Binding" "Command" "Description") :exports results
  (defvar lem-readme-maps
    (mapcar (lambda (x)
              x)
            (list lem-mode-map lem-post-mode-map lem-post-comment-mode-map)))

  (defun lem-readme-where-is-map (symbol maps)
    ""
  ...
  (defun lem-readme-push-rows (symbol)
    (let* ((doc (car
                 (split-string
                  (or (documentation symbol t) "")
                  "\n")))
           (maps lem-readme-maps)
           (binding-codes
            (let ((keys (lem-readme-where-is-map symbol maps)))
...
| /         | lem-switch-to-buffer                   | Switch to a live lem.el buffer.                                           |
